...
class BrokenRecordAgent(RespondAgent[BrokenRecordAgentConfig]):
    ...
    async def respond(
        self, human_input, conversation_id: str, is_interrupt: bool = False
    ) -> tuple[Optional[str], bool]:
        return self.agent_config.message, False

    async def generate_response(
        self, human_input, conversation_id: str, is_interrupt: bool = False, bot_was_in_medias_res: bool = False
    ) -> AsyncGenerator[GeneratedResponse, None]:
        """Returns a generator that yields the agent's response one sentence at a time."""
        yield GeneratedResponse(message=BaseMessage(text=self.agent_config.message), is_interruptible=False)
