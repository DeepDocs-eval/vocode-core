...
SYNTHESIZER_VOICE = os.getenv("SYNTHESIZER_VOICE", "en-US-AriaNeural")
ASR_LANGUAGE = os.getenv("DEEPGRAM_LANGUAGE", "en-US")

synthesizer_config = AzureSynthesizerConfig(voice_name=SYNTHESIZER_VOICE)
transcriber_config = DeepgramTranscriberConfig(language=ASR_LANGUAGE)
```
